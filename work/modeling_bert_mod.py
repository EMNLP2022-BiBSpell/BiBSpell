...
class_model_path = '/home/chenbei/practice/self-attention/work/model/bert-base-chinese-sighan-all-class-finetuned'
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# device='cpu'
print('device:', device)
print('pycharm_work_model_mod')
# class_model_path = 'bert-base-chinese'
tokenizer = BertTokenizer.from_pretrained(class_model_path)
classifierModel = BertForSequenceClassification.from_pretrained(class_model_path,
                                                                num_labels=2).to(device)  # num_labels表示2个分类，错误和正确
...
class IGModel(nn.Module):
    def __init__(self, classifierModel):
        super(IGModel, self).__init__()
        self.classifierModel = classifierModel
        # self.device = 'cpu'

    # 获取梯度
    def get_gradients(self, embeddings, model, attention_mask):
        # 获取句子的embedding
    ...
    def forward(self, input_embeddings, attention_mask=None):  # 主要任务是将上述网络连接起来
        # 获取bert的embedding层
        ...
        for i in range(n):
            ...
            global output
            # 获取梯度
            pred_grad, output = self.get_gradients(input_embeddings, self.classifierModel,
                                                   attention_mask)
        ...
        # p = torch.tensor(scores_new).unsqueeze(dim=2).to(device)
        ...
        # scores = torch.tensor(scores_new,requires_grad=True).to(device)
        scores = torch.tensor(scores_new).to(device)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        query_layer = self.transpose_for_scores(
            mixed_query_layer)  # shape(batch_size,heads,seq_len,vec_dim)(3,12,128,64)
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        batch_size, attention_nums, seq_length, v_dim = key_layer.size()
        # 得到积分梯度的值
        # ig_score_start = time.time()
        if self.layer_index == 0:
            igModel = IGModel(classifierModel)

            ig_scores, ig_output = igModel(hidden_states, attention_mask=attention_mask.squeeze(dim=1).squeeze(dim=1))
            #只保留大于0.5的
            ig_scores = torch.where(ig_scores >= 0.5, ig_scores, torch.zeros_like(ig_scores, dtype=torch.float32))
            ig_scores = ig_scores.unsqueeze(dim=1)
            ig_scores.expand(batch_size, attention_nums, seq_length)
            ig_scores = ig_scores.unsqueeze(dim=2)
            ig_scores = ig_scores.expand(batch_size, attention_nums, seq_length, seq_length)
        # ig_score_end = time.time()
        # print(ig_score_end - ig_score_start)
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer,
                                        key_layer.transpose(-1,
                                                            -2))  # shape:(batch_size,heads,seq_len,seq_len)(3,12,128,128)
        attention_scores = attention_scores + ig_scores
        ...
        attention_probs = nn.Softmax(dim=-1)(attention_scores)  # shape (3,12,128,128)

        # attention_probs_tmp=attention_probs
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # (3,12,128,128)*(3,12,128,64)=(3,12,128,64)

        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # shape(3,128,12,64)

        new_context_layer_shape = context_layer.size()[:-2] + (
            self.all_head_size,)  # shape(batch_size,seq_len,dim)(3,128,768)
...
class BertAttention(nn.Module):
    def __init__(self, config, layer_index):
        super().__init__()
        self.self = BertSelfAttention(config, layer_index)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_index):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = BertAttention(config, layer_index)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config, layer_index) for layer_index in range(config.num_hidden_layers)])
